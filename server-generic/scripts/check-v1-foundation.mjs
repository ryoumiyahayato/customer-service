...
assert.match(compat, /function mapFrontendMessageForVisitor/);
assert.match(compat, /messages\.map\(\(message\) => admin \? mapFrontendMessage\(message\) : mapFrontendMessageForVisitor\(message\)\)/);
assert.match(compat, /messages:\s*page\.messages\.map\(\(message\) => mapFrontendMessageForVisitor\(message\)\)/);
assert.match(compat, /type: 'messages:read'/);
assert.match(compat, /sourceOperatorId/);
assert.match(compat, /createInvite\(context\.db, admin/);
...
assert.match(messages, /ORDER BY created_at \$\{order\}, id \$\{order\}/);
assert.match(messages, /markSessionMessagesRead/);
assert.match(messages, /SELECT status, archived_at, deleted_at, purged_at, history_cleared_at, assigned_operator_id/);
