...
export async function currentAdmin(){const token=(await cookies()).get(COOKIE)?.value; const aid=verifyToken(token); if(!aid)return null; return db.prepare('SELECT id, username, role, must_change_password FROM admins WHERE id=?').get(aid) as Admin|undefined || null;}
