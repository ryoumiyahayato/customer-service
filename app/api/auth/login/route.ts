...
import { recordAuthFailure, recordAuthSuccess } from '@/lib/api-security';
...
export async function POST(req: NextRequest) {
  ...
  if (admin?.is_disabled) { await recordAuthFailure(req, 'auth_login'); return NextResponse.json({ error: '该账户权限已被禁用', disabled: true }, { status: 403 }); }
  if (!admin || !verifyPassword(String(password || ''), admin.password_hash)) { await recordAuthFailure(req, 'auth_login'); return NextResponse.json({ error: 'Invalid credentials' }, { status: 401 }); }
  await recordAuthSuccess(req, 'auth_login');
