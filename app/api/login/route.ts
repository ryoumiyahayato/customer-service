...
export async function POST(req: NextRequest) {
  ...
  if (admin) {
    if (admin.is_disabled) { await recordAuthFailure(req, 'login'); return NextResponse.json({ error: '该账户权限已被禁用', disabled: true }, { status: 403 }); }
    if (!verifyPassword(pass, admin.password_hash)) { await recordAuthFailure(req, 'login'); return NextResponse.json({ error: '账号或密码错误' }, { status: 401 }); }
    await recordAuthSuccess(req, 'login');
  ...
  await recordAuthFailure(req, 'login');
