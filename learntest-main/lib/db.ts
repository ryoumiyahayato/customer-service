...
export async function insertMessage(b: any, senderType: 'VISITOR' | 'OPERATOR', senderId: string) { const t = now(); const msg = { id: id('msg'), session_id: b.sessionId, sender_type: senderType, sender_id: senderId, content: b.content || '', message_type: b.messageType || 'text', image_path: b.imagePath || null, status: 'sent', created_at: t, read_at: null, is_read: 0 }; const sql = getPg(); await sql`INSERT INTO messages VALUES (${msg.id},${msg.session_id},${msg.sender_type},${msg.sender_id},${msg.content},${msg.message_type},${msg.image_path},${msg.status},${msg.created_at},${msg.read_at},${msg.is_read})`; await sql`UPDATE sessions SET status=CASE WHEN status='CLOSED' AND ${senderType}='VISITOR' THEN 'PENDING' ELSE status END, updated_at=${t}, deleted_at=NULL, deleted_by=NULL WHERE id=${b.sessionId}`; return msg; }
