...
assert.match(wrapper, /readJsonWithinLimit/);
assert.match(wrapper, /reader\.cancel\(\)/);
assert.match(wrapper, /total > JSON_REQUEST_MAX_BYTES/);
...
assert.match(secureWorker, /Strict-Transport-Security/);
assert.match(secureWorker, /X-Frame-Options/);
assert.match(secureWorker, /ON CONFLICT\(key\) DO NOTHING/);
assert.match(secureWorker, /reset_at <= \? OR count < \?/);
