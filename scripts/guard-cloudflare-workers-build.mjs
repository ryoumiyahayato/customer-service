...
const ONE_TIME_APPLY_SENTINEL = path.join(root, 'ops', 'cloudflare-auto-apply-once-20260809');
const EXPECTED_ONE_TIME_PENDING = [
  '0010_attachment_claim_token.sql',
  '0010_normalize_unarchive_state.sql',
  '0011_guard_history_clear_ownership.sql',
  '0012_enforce_operator_policy_invariant.sql',
  '0013_structured_runtime_state.sql',
  '0014_operator_preset_messages.sql',
];
...
const localWrangler = path.join(root, 'node_modules', '.bin', `wrangler${isWindows ? '.cmd' : ''}`);

function runWrangler(args) {
  const invocation = wranglerInvocation(existsSync(localWrangler) ? localWrangler : '', npxBin, args);
...
if (pending.length) {
  ...
  console.log('Cloudflare Workers Build gate: approved one-time migrations applied and re-verified.');
}
